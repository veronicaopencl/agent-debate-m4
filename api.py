...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    Applies global rate limits to prevent abuse.
    Federation endpoints have per-key rate limits applied separately.
    """
...
async def lifespan(app: FastAPI):
    """Initialize database on startup and track start time."""
...
# Add privacy headers middleware
...
def health_check():
    ...
    Returns status, database connectivity, and uptime.
    """
...
def submit_turn(
    ...
    """Submit a turn for a participant. Participant ID must be in X-Participant-ID header."""
    participant_id = request.headers.get('X-Participant-ID')
    if not participant_id:
        raise HTTPException(status_code=400, detail="X-Participant-ID header required")
...
def submit_score(
    ...
    """Submit a judge score for a participant. Judge ID must be in X-Judge-ID header."""
    judge_id = request.headers.get('X-Judge-ID')
    if not judge_id:
        raise HTTPException(status_code=400, detail="X-Judge-ID header required")
    # Validate judge
...
def get_debate_log(debate_id: str, db: Session = Depends(get_db)):
    """Get complete debate event log (turns, scores, phase transitions)."""
    debate = db.query(Debate).filter(Debate.id == debate_id).first()
    if not debate:
        raise HTTPException(status_code=404, detail="Debate not found")
    
    # Build chronological log from all debate events
    log_entries = []
    
    # Add debate creation
    log_entries.append({
        "timestamp": debate.created_at.isoformat() if debate.created_at else None,
        "event_type": "debate_created",
        "actor": debate.created_by,
        "data": {
            "title": debate.title,
            "proposition": debate.proposition
        }
    ...
    # Add participant joins
    for p in debate.participants:
        log_entries.append({
            "timestamp": p.joined_at.isoformat() if p.joined_at else None,
    ...
    for turn in debate.turns:
        participant = db.query(Participant).filter(Participant.id == turn.participant_id).first()
        log_entries.append({
            "timestamp": turn.submitted_at.isoformat() if turn.submitted_at else None,
            "event_type": "turn_submitted",
            "actor": participant.name if participant else "unknown",
            "actor_id": turn.participant_id,
            "data": {
                "sequence": turn.sequence_number,
                "phase": turn.phase.value,
                "content_length": turn.content_length,
                "char_limit_violation": turn.char_limit_violation
            }
        })
    
    # Add scores
    for score in debate.scores:
        judge = db.query(Participant).filter(Participant.id == score.judge_id).first()
        ...
        log_entries.append({
            ...
            "actor": judge.name if judge else "unknown",
            "actor_id": score.judge_id,
            "data": {
    ...
    # Add finalization
    if debate.status == DebateStatus.COMPLETE and debate.ended_at:
        log_entries.append({
            "timestamp": debate.ended_at.isoformat(),
    ...
    # Sort by timestamp
    log_entries.sort(key=lambda x: x["timestamp"] or "")
...
def export_debate_json(debate_id: str, db: Session = Depends(get_db)):
    """Export complete debate as JSON (transcript, scores, log)."""
    debate = db.query(Debate).filter(Debate.id == debate_id).first()
    if not debate:
        raise HTTPException(status_code=404, detail="Debate not found")
    ...
    export_data = {
        ...
        "turns": [
            {
                ...
                "phase": t.phase.value,
                "participant_id": t.participant_id,
                "participant_name": db.query(Participant).filter(Participant.id == t.participant_id).first().name,
                "content": t.content,
                "content_length": t.content_length,
                "char_limit_violation": t.char_limit_violation,
        ...
        "scores": [
            {
                "judge_id": s.judge_id,
                "judge_name": db.query(Participant).filter(Participant.id == s.judge_id).first().name,
                "participant_id": s.participant_id,
                "participant_name": db.query(Participant).filter(Participant.id == s.participant_id).first().name,
                "argument_quality": s.argument_quality,
                "evidence_quality": s.evidence_quality,
                "rebuttal_strength": s.rebuttal_strength,
                "clarity": s.clarity,
    ...
    return JSONResponse(
        content=export_data,
        headers={
            "Content-Disposition": f"attachment; filename=debate_{debate_id}.json",
...
def admin_stats(db: Session = Depends(get_db)):
    ...
    stats = {
        "debates": db.query(Debate).count(),
        "agents": db.query(Participant).filter(Participant.participant_type == "agent").count(),
        "keys": db.query(Participant).filter(Participant.api_key != None).count(),
        "tournaments": 0,  # TODO: Add tournament model count
    }
    return stats


@app.get("/api/admin/debates")
def admin_list_debates(limit: int = 50, db: Session = Depends(get_db)):
    """List all debates for admin."""
    debates = db.query(Debate).order_by(Debate.created_at.desc()).limit(limit).all()
...
def leaderboard_page(request: Request):
    """Serve the Elo leaderboard page."""
    return templates.TemplateResponse("leaderboard.html", {"request": request})


# ============== Tournament Bracket View (M6.4) ==============
@app.get("/tournament/{tournament_id}/bracket", response_class=HTMLResponse)
def tournament_bracket_page(request: Request, tournament_id: str):
    """Serve the tournament bracket view page."""
...
def bracket_page(request: Request):
    """Serve the general bracket view page."""
    return templates.TemplateResponse("bracket.html", {"request": request})
